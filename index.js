...
client.once(Events.ClientReady, async (c) => {
  ...
  // ✅ رجعنا التحميل الكامل مثل النظام القديم
  if (guild) {
    console.log('⏳ جاري تحميل أعضاء السيرفر...');
    await guild.members.fetch().catch(console.error);
    console.log('✅ تم تحميل جميع أعضاء السيرفر.');
  }
});
...
async function globalLog(userId, logData) {
  ...
  try {
    ...
    if (channel) {
      let color = 0x808080;
      let title = "📄 سجل العمليات";

      if (logData.type === 'enlist' || logData.type === 'enlistment' || logData.type === 'pre_accept') {
        color = 0x2ECC71;
        title = "✅ " + logData.title;
      } else if (logData.type === 'fail_training' || logData.type === 'reject' || logData.type === 'cert_revoked') {
        color = 0xE74C3C;
        title = "❌ " + logData.title;
      } else if (logData.type === 'fire') {
        color = 0x992D22;
        title = "⚠️ " + logData.title;
      } else if (logData.type === 'promotion' || logData.type === 'discord') {
        color = 0xF1C40F;
        title = "📈 " + logData.title;
      } else if (logData.type === 'cert_granted') {
        color = 0x9B59B6;
        title = "📜 " + logData.title;
      } else if (logData.type === 'armory_issue') {
        color = 0x3498DB;
        title = "🔫 " + logData.title;
      } else if (logData.type === 'academy_grades') {
        color = 0xE67E22;
        title = "🎓 " + logData.title;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setColor(color)
        .addFields(
          { name: "العسكري / المتقدم", value: `${logData.username || 'غير معروف'} (<@${userId}>)`, inline: true },
          { name: "المسؤول", value: logData.actionBy || 'غير معروف', inline: true },
          { name: "التفاصيل", value: logData.details || 'بدون تفاصيل', inline: false }
        )
...
async function hasPermission(user, permKey) {
  ...
  if (user.perms.isTrainer && (
    permKey === 'RECORD_GRADES' ||
    permKey === 'PRE_ACCEPTANCE' ||
    permKey === 'REJECTION_POWER'
  )) return true;

  if (user.perms.isAcademyManager && (
    permKey === 'RECORD_GRADES' ||
    permKey === 'PRE_ACCEPTANCE' ||
    permKey === 'REJECTION_POWER' ||
    permKey === 'FINAL_ACCEPTANCE'
  )) return true;

  if (user.perms.isCertManager && (
    permKey === 'GIVE_CERTS' ||
    permKey === 'OPEN_CLOSE_COURSES' ||
    permKey === 'MANAGE_COURSE_QUESTIONS'
  )) return true;
...
app.use(async (req, res, next) => {
  try {
    ...
    if (req.session.user.perms.isPolice) {
      ...
      req.session.user.customRank = personnelDB[req.session.user.id].rank || personnelDB[req.session.user.id].siteRank || 'مستجد';
...
app.get('/public-rules', async (req, res) => {
  ...
  res.render('content', {
    ...
    section: 'publicRules',
    pageTitle: 'قوانين المواطنين',
    pageDesc: 'الأنظمة والقوانين العامة للمدينة.',
    items: content.publicRules || []
  });
});
...
app.get('/police-rules', async (req, res) => {
  ...
  res.render('content', {
    user: req.session.user,
    section: 'policeRules',
    pageTitle: 'قوانين العسكر',
    pageDesc: 'أنظمة ولوائح القطاع العسكري الداخلي.',
    items: content.policeRules || []
  });
});
...
app.get('/jobs', async (req, res) => {
  ...
  res.render('jobs', {
    user: req.session.user,
    status: appsDB[req.session.user.id] ? appsDB[req.session.user.id].status : 'open',
    appsOpen: settings.appsOpen,
    rules: content.publicRules || [],
    questions
  });
});
...
app.post('/submit', async (req, res) => {
  ...
  for (let key in req.body) {
    if (key.startsWith('q_')) answersData[key] = req.body[key];
  }
  ...
  await globalLog(req.session.user.id, {
    type: 'app_submit',
    title: 'تقديم جديد',
    username: req.session.user.username,
    nationalId: req.body.nationalId,
    actionBy: 'النظام',
    details: 'تم استلام طلب تجنيد جديد'
  });

  const member = await getGuildMember(req.session.user.id);
  if (member) {
    await member.roles.add(process.env.APPLIED_ROLE_ID).catch(() => {});
  }
...
app.get('/admin', async (req, res) => {
  if (!req.session.user || !(req.session.user.perms.isOfficer || req.session.user.perms.isTrainer || req.session.user.perms.isAcademyManager)) {
    return res.redirect('/');
  }
  ...
  res.render('admin', {
    ...
    db: appsDB,
    questions,
    perms: { canPreAccept, canFinalAccept, canReject, canGrade }
  });
});
...
app.get('/academy', async (req, res) => {
  ...
  res.render('academy', {
    ...
    db: appsDB,
    questions,
    perms: { canFinalAccept, canReject, canGrade }
  });
});
...
app.post('/admin/action', async (req, res) => {
  ...
  const target = await getGuildMember(userId);

  if (action === 'accept') {
    if (!await hasPermission(req.session.user, 'PRE_ACCEPTANCE')) return res.redirect('/admin');

    if (target) {
      await target.roles.add(process.env.INITIAL_ACCEPT_ROLE_ID).catch(() => {});
      await target.roles.remove(process.env.APPLIED_ROLE_ID).catch(() => {});
    }
    ...
    if (target) {
      await target.roles.remove([process.env.APPLIED_ROLE_ID, process.env.INITIAL_ACCEPT_ROLE_ID]).catch(() => {});
    }

    appsDB[userId].status = action === 'reject' ? 'rejected_initial' : 'failed_academy';
    appsDB[userId].rejectReason = reason;
...
app.post('/admin/enlist', async (req, res) => {
  ...
  if (!targetData || (targetData.status !== 'academy' && targetData.status !== 'failed_academy')) {
  ...
  try {
    ...
    if (member) {
      await member.roles.remove([process.env.APPLIED_ROLE_ID, process.env.INITIAL_ACCEPT_ROLE_ID]).catch(e => {
        console.log("خطأ في سحب الرتب القديمة:", e);
      });

      if (process.env.ENLISTED_ROLE_ID) {
        await member.roles.add(process.env.ENLISTED_ROLE_ID).catch(e => {
          console.log("خطأ في إعطاء رتبة الأفراد:", e);
        });
      }
    ...
    await globalLog(userId, {
      ...
      nationalId: targetData.personalInfo?.nationalId,
      actionBy: req.session.user.username,
      details: 'تم التجنيد النهائي وإضافته إلى الأفراد'
    });
...
app.get('/certificates', async (req, res) => {
  ...
  const myRank = req.session.user.customRank || 'مستجد';
  ...
  for (let key in CERT_TYPES) {
    const cert = CERT_TYPES[key];
    const reqIndex = RANKS_LADDER.indexOf(cert.minRank);

    if (
      rankIndex >= reqIndex &&
      ((cert.type === 'enlisted' && rankIndex <= 3) || (cert.type === 'nco' && rankIndex >= 4))
    ) {
...
app.post('/certificates/apply', async (req, res) => {
  ...
  certsDB.apps[req.session.user.id][certId] = {
    status: 'pending',
...
app.get('/system/certificates', async (req, res) => {
  if (!req.session.user || !(req.session.user.perms.isOfficer || await hasPermission(req.session.user, 'GIVE_CERTS'))) {
    return res.redirect('/');
  }
  ...
  res.render('cert_admin', {
    user: req.session.user,
    certTypes: CERT_TYPES,
    certsDB,
    perms: { canManageCourses, canManageQuestions }
  });
});
...
app.post('/system/certificates/action', async (req, res) => {
  ...
  if (action === 'accept') {
    if (!personnelDB[userId]) personnelDB[userId] = { rank: 'مستجد', certs: [] };
    ...
    await globalLog(userId, {
      type: 'cert_granted',
      title: 'منح شهادة',
      username: certsDB.apps[userId]?.[certId]?.username || targetData.realName || 'عسكري',
      nationalId: targetData.nationalId,
      actionBy: req.session.user.username,
      details: `حصل على ${CERT_TYPES[certId].name}`
    });

    await db.addNotification(userId, `🎖️ مبروك! تم منحك ${CERT_TYPES[certId].name}.`, 'success');
    if (certsDB.apps[userId]) delete certsDB.apps[userId][certId];
  } else if (action === 'reject') {
    if (certsDB.apps[userId] && certsDB.apps[userId][certId]) {
...
app.get('/system', async (req, res) => {
  ...
  const guild = getGuild();
  const members = guild ? guild.members.cache : new Map();

  const counts = {
    officers: guild ? members.filter(m => process.env.OFFICERS_ROLE_ID && m.roles.cache.has(process.env.OFFICERS_ROLE_ID)).size : 0,
    ncos: guild ? members.filter(m => process.env.NCO_ROLE_ID && m.roles.cache.has(process.env.NCO_ROLE_ID)).size : 0,
    enlisted: guild ? members.filter(m => process.env.ENLISTED_ROLE_ID && m.roles.cache.has(process.env.ENLISTED_ROLE_ID)).size : 0
  };
...
app.post('/system/reset-rejected', async (req, res) => {
  ...
  for (const userId in appsDB) {
    if (
      appsDB[userId].status === 'rejected' ||
      appsDB[userId].status === 'rejected_initial' ||
      appsDB[userId].status === 'failed_academy'
    ) {
      delete appsDB[userId];
      count++;
    }
...
app.get('/system/questions', async (req, res) => {
  ...
  const questions = await db.get('questions', []);
  res.render('questions', { user: req.session.user, questions });
});
...
app.get('/system/personnel', async (req, res) => {
  ...
  const list = policeMembers.map(m => {
    ...
    return {
      ...
      realName: dbData.realName || m.user.username,
...
app.post('/system/personnel/action', async (req, res) => {
  ...
  const targetData = personnelDB[targetId] || {
    rank: "مستجد",
    nationalId: "غير مسجل",
    realName: target ? target.user.username : "عسكري",
    certs: [],
    delegatedPerms: []
  };
  ...
  if (action === 'fire') {
    if (target) {
      await target.roles.remove([
        ...
        process.env.NCO_ROLE_ID,
        process.env.OFFICERS_ROLE_ID
      ]).catch(() => {});
    }

    delete personnelDB[targetId];

    await globalLog(targetId, {
      type: 'fire',
      title: 'فصل وطي قيد',
      username: targetData.realName,
      nationalId: targetData.nationalId,
      actionBy: req.session.user.username,
      details: `السبب: ${reason}`
    });
    ...
    if (target) {
      ...
      await target.roles.remove(allRoleIds).catch(() => {});

      let newRoleId = null;
      let roleLabel = 'أفراد';

      if (newDiscordRole === 'enlisted') {
        newRoleId = process.env.ENLISTED_ROLE_ID;
        roleLabel = 'أفراد';
      } else if (newDiscordRole === 'nco') {
        newRoleId = process.env.NCO_ROLE_ID;
        roleLabel = 'ضباط صف';
      } else if (newDiscordRole === 'officer') {
        newRoleId = process.env.OFFICERS_ROLE_ID;
        roleLabel = 'ضباط';
      }
      ...
      await globalLog(targetId, {
        type: 'discord',
        title: 'تحديث فئة ديسكورد',
        username: targetData.realName,
        nationalId: targetData.nationalId,
        actionBy: req.session.user.username,
        details: `تحديث الفئة إلى: ${roleLabel}`
      });
    ...
    const oldRank = personnelDB[targetId].rank || personnelDB[targetId].siteRank || "مستجد";

    if (
      oldRank === newSiteRank &&
      personnelDB[targetId].nationalId === newNationalId &&
      personnelDB[targetId].realName === newRealName
    ) {
      personnelDB[targetId].delegatedPerms = Array.isArray(delegatedPerms)
        ? delegatedPerms
        : (delegatedPerms ? [delegatedPerms] : []);
    ...
    delete personnelDB[targetId].siteRank;
    ...
    await globalLog(targetId, {
      type: 'promotion',
      title: 'تحديث بيانات ورتبة',
      username: personnelDB[targetId].realName,
      nationalId: personnelDB[targetId].nationalId,
      actionBy: req.session.user.username,
      details: `تعديل الرتبة من ${oldRank} إلى ${newSiteRank}`
    });
...
app.post('/system/personnel/revoke-cert', async (req, res) => {
  ...
  if (targetData && targetData.certs) {
    ...
    await globalLog(targetId, {
      ...
      nationalId: targetData.nationalId,
      actionBy: req.session.user.username,
      details: `تم سحب شهادة: ${CERT_TYPES[certId] ? CERT_TYPES[certId].name : certId}`
    });
...
app.post('/system/personnel/grant-cert-direct', async (req, res) => {
  ...
  if (personnelDB[targetId]) {
    ...
    await globalLog(targetId, {
      type: 'cert_granted',
      title: 'منح شهادة استثنائي',
      username: personnelDB[targetId].realName,
      nationalId: personnelDB[targetId].nationalId,
      actionBy: req.session.user.username,
      details: `منح ${CERT_TYPES[certId].name} مباشرة من الإدارة`
    });
...
app.post('/custody/weapons/issue', async (req, res) => {
  ...
  const log = {
    receiverDiscordId: req.body.discordId,
    receiverName: req.body.receiverName,
    nationalId: req.body.nationalId,
    weaponType: req.body.weaponType,
    serialNumber: req.body.serialNumber,
    issuerName: req.session.user.username,
    date: new Date().toLocaleString('ar-SA')
  };
  ...
  await globalLog(req.body.discordId, {
    type: 'armory_issue',
    title: 'صرف عهدة (سلاح)',
    username: req.body.receiverName,
    nationalId: req.body.nationalId,
    actionBy: req.session.user.username,
    details: `سلاح: ${req.body.weaponType} | S/N: ${req.body.serialNumber}`
  });
...
app.post('/custody/vehicles/issue', async (req, res) => {
  ...
  await globalLog(req.body.discordId, {
    type: 'armory_issue',
    title: 'صرف عهدة (مركبة)',
    username: req.body.receiverName,
    nationalId: req.body.nationalId,
    actionBy: req.session.user.username,
    details: `مركبة: ${req.body.vehicleType} | لوحة: ${req.body.plateNumber}`
  });
...
app.post('/system/custody/return', async (req, res) => {
  ...
  if (req.body.type === 'weapon' && custodyDB.weaponLogs) {
    custodyDB.weaponLogs.splice(req.body.index, 1);
  } else if (req.body.type === 'vehicle' && custodyDB.vehicleLogs) {
    custodyDB.vehicleLogs.splice(req.body.index, 1);
  }
...
app.get('/system/ledger', async (req, res) => {
  ...
  const ledger = await db.get('global_ledger', []);
  res.render('ledger', { user: req.session.user, ledger });
});
...
app.get('/armory', async (req, res) => {
  ...
  const myWeapons = (custody.weaponLogs || [])
    .map((log, index) => ({ ...log, originalIndex: index }))
    .filter(l => l.receiverDiscordId === req.session.user.id);

  const myVehicles = (custody.vehicleLogs || [])
    .map((log, index) => ({ ...log, originalIndex: index }))
    .filter(l => l.receiverDiscordId === req.session.user.id);
